...
NUM_EPOCHS = 100
VAL_EVERY = 1
ACCUMULATION_STEPS = 2 # batch size를 많이 키울 수 없어 Gradient Accumulation 진행
...
for i, (x, y) in enumerate(gkf.split(pngs, ys, groups)):
    ...
    if i == 2: # 각자 서버 번호대로 validation fold 설정해서 돌리기 
        valid_filenames += list(pngs[y])
        valid_labelnames += list(jsons[y])
        print(sorted(valid_filenames))
    else:
...
def bce_dice_loss(pred, target, bce_weight=LOSS_WEIGHT):
...
def save_model(model, file_name='DeepLabv3+_CRF_lr1e-3_combined_loss_1536_fold0.pt'):
...
def train(model, data_loader, val_loader, optimizer, scheduler):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for step, (images, masks) in enumerate(data_loader):
            ...
            images, masks = images.cuda(), masks.cuda()
            # Autocast를 이용해 mixed precision (FP16)
            with autocast():
                # inference
                outputs = model(images)
                # loss 계산
        ...
        scheduler.step() # Learning rate scheduler 도입
